# 📦 Деплой партнёрской системы

## 1️⃣ Настройка Supabase (БД)

### Создание таблиц:

1. Откройте ваш проект в **Supabase Dashboard**
2. Перейдите в **SQL Editor**
3. Скопируйте содержимое файла `supabase/migrations/001_create_affiliate_tables.sql`
4. Выполните SQL-запрос (Execute SQL)
5. Проверьте что таблицы созданы: **Table Editor** → должны появиться:
   - `partners`
   - `partner_clicks`
   - `partner_conversions`
   - `partner_payouts`

### Создание тестовых партнёров (опционально):

В SQL Editor выполните:

```sql
INSERT INTO partners (id, name, website, commission_rate, notes) VALUES
  ('dnsshop', 'DNS Shop', 'https://dnsshop.ru', 200.00, 'Крупная сеть электроники'),
  ('test-partner', 'Тестовый партнёр', NULL, 10.00, 'Для тестирования на проде');
```

---

## 2️⃣ Настройка переменных на хостинге (Vercel)

### Обязательные переменные:

Перейдите в **Vercel Dashboard** → Ваш проект → **Settings** → **Environment Variables**

Добавьте:

```bash
# Supabase (уже должны быть)
NEXT_PUBLIC_SUPABASE_URL=https://your-project.supabase.co
NEXT_PUBLIC_SUPABASE_ANON_KEY=your-anon-key
SUPABASE_SERVICE_ROLE_KEY=your-service-role-key

# URL сайта для API запросов (КРИТИЧЕСКИ ВАЖНО!)
NEXT_PUBLIC_SITE_URL=https://youwow.ru
```

**⚠️ ВАЖНО:** Переменная `NEXT_PUBLIC_SITE_URL` **обязательна** для production! Без неё:
- Middleware не сможет записывать клики в БД (SSL ошибки)
- Трекинг конверсий не будет работать
- Партнёрская система не будет функционировать

На Vercel обязательно установите: `NEXT_PUBLIC_SITE_URL=https://youwow.ru`

### Переменные уже есть в проекте:

Эти переменные уже должны быть настроены, просто проверьте:
- ✅ `NEXT_PUBLIC_SUPABASE_URL`
- ✅ `NEXT_PUBLIC_SUPABASE_ANON_KEY`
- ✅ `SUPABASE_SERVICE_ROLE_KEY`

Если их нет - добавьте из Supabase Dashboard → Project Settings → API.

### Для тестирования с оплатой 10 рублей:

Добавьте переменную (только для тестов, не на production!):

```
YOOKASSA_MIN_TEST_PRICE=1000
```

**Важно:**
- Указывается в **КОПЕЙКАХ**: `1000` = 10₽, `59000` = 590₽
- Если переменная **не указана** → цена будет 590₽ (как в продакшене)
- Если указана `YOOKASSA_MIN_TEST_PRICE=1000` → цена будет 10₽
- На production **не ставьте** эту переменную, чтобы цена была реальной!

**Примеры:**
- 10₽: `YOOKASSA_MIN_TEST_PRICE=1000`
- 50₽: `YOOKASSA_MIN_TEST_PRICE=5000`
- 100₽: `YOOKASSA_MIN_TEST_PRICE=10000`

---

## 3️⃣ Изменение комиссии партнёра

### Вариант 1: Через SQL Editor в Supabase

Установить комиссию **10 рублей** для партнёра `test-partner`:

```sql
UPDATE partners
SET commission_rate = 10.00
WHERE id = 'test-partner';
```

### Вариант 2: При создании партнёра

```sql
INSERT INTO partners (id, name, commission_rate, is_active)
VALUES ('new-partner', 'Новый партнёр', 10.00, true);
```

### Как это работает:

- Комиссия хранится в таблице `partners` в поле `commission_rate`
- Значение по умолчанию: **200₽**
- Каждый партнёр может иметь **свою комиссию**
- При записи конверсии берётся комиссия из БД для конкретного партнёра

**Пример:**
- Партнёр `dnsshop`: комиссия 200₽
- Партнёр `test-partner`: комиссия 10₽
- Партнёр `vip-partner`: комиссия 300₽

---

## 4️⃣ Проверка деплоя

### После деплоя проверьте:

1. **Откройте сайт:** `https://youwow.ru/song?partner=test-partner`

2. **Проверьте cookie:**
   - DevTools → Application → Cookies
   - Должен быть cookie `youwow_partner`

3. **Создайте тестовый заказ:**
   - Заполните форму
   - Оплатите тестовой картой
   - Дождитесь генерации

4. **Проверьте в Supabase:**
   - Table Editor → `partner_clicks` → должна быть запись
   - Table Editor → `partner_conversions` → после генерации песни

5. **Проверьте логи в Vercel:**
   - Vercel Dashboard → Deployments → Functions → Logs
   - Должны быть логи: `[Affiliate] Conversion tracked successfully`

---

## 5️⃣ Создание реальных партнёров

### Через SQL Editor:

```sql
INSERT INTO partners (id, name, website, payment_info, commission_rate, notes)
VALUES (
  'partnerId',           -- Уникальный ID (латиница, без пробелов)
  'Название партнёра',   -- Отображаемое имя
  'https://site.ru',     -- Сайт партнёра
  'Карта СберБанк 5536 9137 XXXX XXXX, Иван Иванов', -- Реквизиты
  250.00,                -- Комиссия в рублях
  'VIP-партнёр, повышенная комиссия' -- Заметки
);
```

### Формат партнёрской ссылки:

```
https://youwow.ru/song?partner=partnerId
https://youwow.ru/santa?partner=partnerId
https://youwow.ru/?partner=partnerId
```

---

## 6️⃣ Мониторинг

### Проверка работы системы:

```sql
-- Количество кликов за сегодня
SELECT partner_id, COUNT(*) as clicks
FROM partner_clicks
WHERE clicked_at >= CURRENT_DATE
GROUP BY partner_id;

-- Количество конверсий за сегодня
SELECT partner_id, COUNT(*) as conversions, SUM(commission) as total_commission
FROM partner_conversions
WHERE converted_at >= CURRENT_DATE
GROUP BY partner_id;

-- Топ партнёров по конверсиям
SELECT p.name, COUNT(c.id) as conversions, SUM(c.commission) as earned
FROM partners p
LEFT JOIN partner_conversions c ON p.id = c.partner_id
WHERE c.converted_at >= CURRENT_DATE - INTERVAL '7 days'
GROUP BY p.name
ORDER BY conversions DESC;
```

---

## 🔥 Быстрый старт для продакшена

1. **Выполните SQL миграцию** в Supabase
2. **Создайте тестового партнёра** с ID `test-partner` и комиссией 10₽
3. **Задеплойте** на Vercel (автоматически при push на GitHub)
4. **Протестируйте** ссылку: `https://youwow.ru/song?partner=test-partner`
5. **Проверьте** таблицы в Supabase после покупки

**Всё должно работать автоматически!** 🚀

---

## ❓ Troubleshooting

### Конверсии не записываются?

1. Проверьте логи в Vercel Functions
2. Убедитесь что переменная `NEXT_PUBLIC_SITE_URL` установлена
3. Проверьте что таблица `partners` содержит партнёра с нужным ID
4. Убедитесь что партнёр активен (`is_active = true`)

### Cookie не сохраняются?

1. Проверьте что middleware работает (логи в Vercel)
2. Убедитесь что параметр `?partner=xxx` корректный
3. Проверьте настройки браузера (разрешены ли cookie)

### Ошибка "Partner not found"?

Партнёр с таким ID не существует в таблице `partners`. Создайте его через SQL Editor.
